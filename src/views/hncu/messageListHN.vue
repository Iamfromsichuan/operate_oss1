  ...
  export default {
    ...
    methods: {
      ...
      search(no) {
        ...
        util.ajax.get(util.baseUrl + '/hncu/sms/mt/listV2', {
          params: this.filter
        })
        .then(function(res){
      ...
      download() {
        window.location.href = util.baseUrl + '/hncu/sms/mt/exportV2?token='+Cookies.get('token') + util.parseParam(this.filter)
      }
    },
    mounted() {
