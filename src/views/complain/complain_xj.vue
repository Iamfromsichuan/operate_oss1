<template>
  <div class="complain_xj">
    ...
    <Table v-show="tagIndex != 8" :columns="columns" :data="datas" :loading="dataLoading" @on-selection-change='selectItem' size="small"></Table>

    <div class="statistic" v-show="tagIndex == 8">
      <Statistic :dataArr='StatisticDatas' :dataLoading='dataLoading'></Statistic>
    </div>

    <br>
    <br>
    <div class="page" v-show="tagIndex != 8">
    ...
    <Modal v-model="modalChangeStatus" width="360">
      ...
      <div style="text-align:center;overflow: hidden">
        <p v-if="changeType != '一次退款' && changeType != '二退查询' && changeType != '二次退费'">确定要修改 <span style="font-weight: bold">{{changeTips}}</span> 状态吗？</p>
        <p v-if="changeType == '一次退款' || changeType == '二退查询' || changeType == '二次退费'">对选定的{{checkedDatas.length}}条记录提交 <span style="font-weight: bold">{{batchOperationType}}</span> 并导出清单</p>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                // 上传
                uploadUrl: `${util.baseUrl}/xjcu/refund/uploadFile.do?token=${Cookies.get('token')}`,
                fileType: {
                    fileType: ''
                },
                ruleValidate: {
                ...
                // 筛选条件相关
                tagList: ['投诉查询', '回单跟进', '号码屏蔽', '退订跟进', '内部退款', '一次退款', '二退查询', '二次退费', '退费统计'],
                collapseName: '',
                ...
                columns: [
                    {
                        ...
                        render: (h, params) => {
                            return h('div', {
                                ...
                                h('Button', {
                                    props: {
                                        type: params.row.internalRefundStatus == '无' || params.row.refundScreenshot != '' ? 'ghost' : 'warning',
                                        size: 'small'
                                    },
                                    on: {
                                        click: () =>{
        ...
        methods: {
            ...
            gettagIndex(index) {
                ...
                if(index === 1 || index === 5 || index === 7) {
                    ...
                    index === 1 ? this.batchOperationType = '回邮' : index === 5 ? this.batchOperationType = '一次退款' : index === 6 ? this.batchOperationType = '二退查询' : this.batchOperationType = '二次退费'
                    index === 1 ? this.changeType = '回邮' : index === 5 ? this.changeType = '一次退款' : index === 6 ? this.changeType = '二退查询' : this.changeType = '二次退费'
                }else {
                ...
                var _filter = {
                    ...
                    refundSource: this.filter.refundSource
                }
                switch (index) {
                    case -1:
                        this.filter = _filter
            ...
            search(no) {
                //--------------------下,此区域  由于业务的需求，在没有选择日期时，默认当前时间向前一周
                var _that = this;
                this.filter.pageNo = no
                this.dataLoading = true
                this.orderNos = ''
                this.checkedDatas = []
                if(this.tagIndex == 8 && !this.filter.complainStartTime){
                  var myDate = new Date()
                  var sec = myDate.getTime();
                  var seven = 7*24*60*60*1000;
                  var second = sec-seven
                  ...
                  function timeFormat(timestamp){
                    ...
                    function timeFormatend(timestamp){
                        //timestamp是整数，否则要parseInt转换,不会出现少个0的情况
                        var time = new Date(timestamp);
                        var year = time.getFullYear();
                        var month = time.getMonth()+1;
                        ...
                        var minutes = time.getMinutes();
                        var seconds = time.getSeconds();
                        return year+'-'+add0(month)+'-'+add0(date)+' '+ "00:00:00";
                    };
                  var myDate = timeFormat(sec);
                  var myDated = timeFormatend(sec-seven);
                  this.filter.complainStartTime = myDated
                  this.filter.complainEndTime = myDate
                }
                //-------------------------------------------上


                var _url = this.tagIndex == 8 ? '/xjcu/refund/refundStatisticsList.do' : '/xjcu/refund/list'
            ...
            changeNos() {
                ...
                if(this.tagIndex == 5) {
                    ...
                    this.changeType = '二退查询'
                } else if (this.tagIndex == 7) {
                    this.changeType = '二次退费'
                }
            ...
            changeStatus() {
                ...
                util.ajax.get(util.baseUrl + this.changeUrl, {
                    ...
                    .then(function(res){
                        ...
                        //  twoRefundMoneyStatus出现了问题  在此做了修改-----修改内容为   重置默认状态
                        _that.twoRefundMoneyStatus = 0
                        if(res.data.status == ERR_OK) {
                            if(_that.changeType == '一次退款' || _that.changeType == '二退查询' || _that.changeType == '二次退费') {
                                var _url = _that.changeType == '一次退款' ? '/xjcu/refund/editOneRefundExport.do' : '/xjcu/refund/editTwoRefundExport.do'
                                let url = `${util.baseUrl}${_url}?orderNos=${res.data.data}&token=${Cookies.get('token')}`
            ...
            addShow() {
                ...
                this.formItem = {
                    ...
                    oneRefundStatus: '待提交', // 一次退款状态
                    oneRefundMoney: 0, // 一次退款金额
            ...
            checkRefundType() {
                ...
                if(this.radio == 'inner'){
                    ...
                    if(this.formItem.oneRefundStatus == '待提交' && this.formItem.oneRefundMoney === "" || this.formItem.oneRefundStatus == '待提交' && this.formItem.oneRefundMoney === 0) {
            ...
            downloadFile(fileName) {
                window.location.href = `${util.baseUrl}/xjcu/refund/downFile.do?fileName=${fileName}&token=${Cookies.get('token')}`
            },
            initData(dataArr) {
                var _total = {
                    complainDate: '合计',
                    ...
                    internalTotalMoney: 0,

                    oneLowTotalCount: 0,
                    oneLowTotalMoney: 0,
                    internalLowTotalCount: 0,
                    internalLowTotalMoney: 0,

                    oneMonthTotalCount: 0,
                    oneMonthTotalMoney: 0,
                    internalMonthTotalCount: 0,
                    internalMonthTotalMoney: 0,

                    oneContentTotalCount: 0,
                    oneContentTotalMoney: 0,
                    internalContentTotalCount: 0,
                    internalContentTotalMoney: 0,

                    oneOtherTotalCount: 0,
                    oneOtherTotalMoney: 0,
                    internalOtherTotalCount: 0,
                ...
                dataArr.map(item => {
                    var _obj = {
                        oneTotalCount: `${item.oneContentTotalCount + item.oneLowTotalCount + item.oneMonthTotalCount + item.oneOtherTotalCount}`,
                        oneTotalMoney: `${item.oneContentTotalMoney + item.oneLowTotalMoney + item.oneMonthTotalMoney + item.oneOtherTotalMoney}`,
                        internalTotalCount: `${item.internalContentTotalCount + item.internalLowTotalCount + item.internalMonthTotalCount + item.internalOtherTotalCount}`,
                        internalTotalMoney: `${item.internalContentTotalMoney + item.internalLowTotalMoney + item.internalMonthTotalMoney + item.internalOtherTotalMoney}`
                    }
                    item = Object.assign(item, _obj)
                })
                this.StatisticDatas = dataArr
            }
        },
...
<style>
  .complain_xj .vertical-center-modal {
    display: flex;
    align-items: center;
  ...
  .complain_xj .ivu-modal-body{
    overflow: hidden;
  }
  .complain_xj .ivu-poptip-inner{
    white-space: normal;
    font-size: 14px;
  }
  .complain_xj .ivu-checkbox-inner{
    border: 1px solid #2196F3;
  }
  .complain_xj .ivu-table th{
    overflow: visible;
  }
  .complain_xj .ivu-table .ivu-btn-ghost{
  ...
  .complain_xj .ivu-table{
    color: #000;
  }
  .complain_xj .ivu-table .ivu-table-cell .ivu-btn{
    min-width: 28px;
    min-height: 24px;
  }
